...
def _handle_benchmark(args: list[str]) -> int:
    if not args or args[0] in {"-h", "--help"}:
        ...
        subparsers.add_parser(
            "track2p-bootstrap",
            help="Run Track2p global assignment with self-bootstrapped registration",
        )
        subparsers.add_parser(
            "track2p-sweep",
    ...
    if args[0] == "track2p-bootstrap":
        from bayescatrack.experiments.track2p_bootstrap_benchmark import (
            main as _track2p_bootstrap_benchmark_main,
        )

        return int(_track2p_bootstrap_benchmark_main(args[1:]))
