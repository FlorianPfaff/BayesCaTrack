...
def export_loso_calibration_csv(
    ...
    *,
    ...
    The exported rows are out-of-fold: for each held-out subject, the logistic
    association model is trained on all other subjects and then evaluated on all
    candidate ROI pairs from the held-out subject. The CSV is intended for Brier
    score, expected calibration error, reliability diagrams, and error ledgers.
    """
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Declared reference type; manual-gt is the default for calibration export",
    )
    parser.add_argument(
        "--allow-track2p-as-reference-for-smoke-test",
        action="store_true",
        help="Permit non-manual references for plumbing smoke tests only",
    )
    ...
    parser.add_argument("--max-gap", type=int, default=2, help="Maximum forward session gap for candidate edges")
    parser.add_argument("--transform-type", default="affine", choices=("affine", "rigid", "none"))
    parser.add_argument("--curated-only", action="store_true", help="Use only reference tracks marked curated")
