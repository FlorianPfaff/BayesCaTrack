...
class SolverPriorCandidate:
    ...
    def config_for(self, config: Track2pBenchmarkConfig) -> Track2pBenchmarkConfig:
        ...
        return replace(
            ...
            cost_threshold=(
                None if self.cost_threshold is None else float(self.cost_threshold)
            ),
        )
...
class SolverPriorTuningResult:
    ...
    def config_with_best_priors(
        self, config: Track2pBenchmarkConfig
    ) -> Track2pBenchmarkConfig:
...
def tune_solver_priors(
    ...
    *,
    ...
    best_scores = _score_candidate(
        best_candidate, prepared, config=config, solver=solver
    )
    for candidate in candidates[1:]:
        scores = _score_candidate(candidate, prepared, config=config, solver=solver)
        if _objective_tuple(scores, search.objective) > _objective_tuple(
            best_scores, search.objective
        ):
...
def run_track2p_loso_solver_priors(
    ...
    *,
    ...
    if config.split != "leave-one-subject-out":
        raise ValueError(
            "LOSO solver-prior tuning requires split='leave-one-subject-out'"
        )
    if config.cost == "calibrated":
        raise ValueError("Use the calibrated LOSO path for cost='calibrated'")

    subjects = tuple(
        _load_subject_data(subject_dir, config=config)
        for subject_dir in discover_subject_dirs(config.data)
    )
    if len(subjects) < 2:
        raise ValueError(
    ...
    for held_out_index, held_out in enumerate(subjects):
        ...
        scores: dict[str, float | int | str] = {
            **_score_prediction_against_reference(
                predicted, held_out.reference, config=solve_config
            ),
            **tuning.score_fields(),
        }
        folds.append(
...
def main(argv: list[str] | None = None) -> int:
    ...
    search = SolverPriorSearchConfig(
        ...
        gap_penalties=parse_nonnegative_list(
            args.gap_penalties, name="--gap-penalties"
        ),
    ...
    *,
    ...
    for subject in subjects:
        ...
        subject_scores.append(
            _score_prediction_against_reference(
                predicted, subject.reference, config=fold_config
            )
        )
...
def _aggregate_scores(
    subject_scores: Sequence[Mapping[str, float | int]],
) -> dict[str, float | int]:
    aggregate: dict[str, float | int] = {"subjects": len(subject_scores)}
    for prefix in ("pairwise", "complete_track"):
        tp = sum(
            int(scores.get(f"{prefix}_true_positives", 0)) for scores in subject_scores
        )
        fp = sum(
            int(scores.get(f"{prefix}_false_positives", 0)) for scores in subject_scores
        )
        fn = sum(
            int(scores.get(f"{prefix}_false_negatives", 0)) for scores in subject_scores
        )
    ...
    aggregate["mean_f1"] = 0.5 * (
        float(aggregate["pairwise_f1"]) + float(aggregate["complete_track_f1"])
    )
    return aggregate


def _objective_tuple(
    scores: Mapping[str, float | int], objective: SolverPriorObjective
) -> tuple[float, float, float]:
    return (
        float(scores[objective]),
        float(scores["complete_track_f1"]),
        float(scores["pairwise_f1"]),
    )


def _load_subject_data(
    subject_dir: Path, *, config: Track2pBenchmarkConfig
) -> SubjectSolverPriorData:
    ...
    if len(sessions) != reference.n_sessions:
        raise ValueError(
            f"Subject {subject_dir.name!r} has {len(sessions)} loaded sessions but {reference.n_sessions} reference sessions"
        )
    return SubjectSolverPriorData(
...
def parse_threshold_list(raw: str) -> tuple[float | None, ...]:
    ...
    for token in _split(raw, name="--cost-thresholds"):
        values.append(
            ...
            else _parse_float(token, name="--cost-thresholds")
        )
